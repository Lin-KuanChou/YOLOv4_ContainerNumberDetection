...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        for i, c in list(enumerate(out_classes)):
            ...
            crop_image = image.crop([left, top, right, bottom+6])
            crop_image = np.array(crop_image)
            r, ocr_image = cv2.threshold(crop_image, 215, 255, cv2.THRESH_BINARY) #反二值化處理

            pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
            text = pytesseract.image_to_string(ocr_image, lang='eng')
            
            text.replace(" ", "")
            text = text.strip()
            text = ''.join(char for char in text if char.isalnum())

            # label = '{} {:.2f} {}'.format(predicted_class, score, text)
            label = '{}'.format(text)
